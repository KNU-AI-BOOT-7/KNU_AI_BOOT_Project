...
- 이재현
- 장지훈(
- 최세민(AI학습 및 전처리, 얼굴 담당)

## 서비스 정의

VoiceGuard AI는 통화 중 발생하는 보이스피싱 위험 신호를 실시간으로 탐지하는 서비스입니다.

- 프론트엔드는 통화 시작 시 WebSocket을 연결하고, 이후 mp3/wav/m4a 오디오 chunk를 백엔드로 전송합니다.
...
- 실시간 mp3/wav/m4a 오디오 chunk WebSocket 수신
...
backend/
  app/
    ...
    services/
      audio_transcriber.py    # mp3/wav/m4a 전사, base64, 오디오 포맷 정규화
...
3. 클라이언트가 3~4초 단위 mp3/wav/m4a 바이너리 frame을 전송합니다.
...
통화 시작 이후 클라이언트는 3~4초 단위의 mp3/wav/m4a 오디오를 WebSocket binary frame으로 전송합니다.
